    ...
    <div id="app">
                        ...
                        <tr v-for="il in active_invite_links" :key="il.id">
                            ...
                            <td>
                                <b-dropdown no-caret right>
                                    ...
                                    <b-dropdown-item @click="copyToClipboard(il, true)"> <i class="fas fa-link"></i> {{ $t("Copy Link") }} </b-dropdown-item>

                                    <b-dropdown-item @click="copyToClipboard(il, false)"> <i class="far fa-clipboard"></i> {{ $t("Copy Token") }} </b-dropdown-item>

                                    <b-dropdown-item @click="deleteInviteLink(il)"> <i class="fas fa-trash-alt"></i> {{ $t("Delete") }} </b-dropdown-item>
                                </b-dropdown>
                            </td>
                        </tr>
                    </table>
        ...
        <div class="row mt-4" v-if="space !== undefined">
            <div class="col col-12">
                ...
                <label>{{ $t("Image") }}</label>
                <generic-multiselect :initial_single_selection="space.image" :model="Models.USERFILE" :multiple="false" @change="space.image = $event.val"></generic-multiselect>
                <br />

                <label>{{ $t("FoodInherit") }}</label>
                <generic-multiselect :initial_selection="space.food_inherit" :model="Models.FOOD_INHERIT_FIELDS" @change="space.food_inherit = $event.val"> </generic-multiselect>
                <span class="text-muted small">{{ $t("food_inherit_info") }}</span
                ><br />

                <a class="btn btn-success" @click="updateSpace()">{{ $t("Save") }}</a>
                <br /><br />
                <a class="btn btn-warning mt-1" @click="resetInheritance()">{{ $t("reset_food_inheritance") }}</a
                ><br />
        ...
        <div class="row mt-4">
            <div class="col col-12">
                ...
                <br />
                <a class="btn btn-danger" :href="resolveDjangoUrl('delete_space', ACTIVE_SPACE_ID)">{{ $t("Delete") }}</a>
            </div>
        </div>
        ...
        <generic-modal-form
            :model="Models.INVITE_LINK"
            :action="Actions.CREATE"
            :show="show_invite_create"
            @finish-action="
...
export default {
    ...
    mounted() {
        ...
        apiFactory.listUserSpaces(1, 25).then((r) => {
            //TODO build proper pagination
    ...
    methods: {
        ...
        updateSpace: function () {
            let apiFactory = new ApiApiFactory()
            apiFactory
                .partialUpdateSpace(window.ACTIVE_SPACE_ID, this.space)
                .then((r) => {
        ...
        updateUserSpace: function (userSpace) {
            let apiFactory = new ApiApiFactory()
            apiFactory
                .partialUpdateUserSpace(userSpace.id, userSpace)
                .then((r) => {
                    StandardToasts.makeStandardToast(this, StandardToasts.SUCCESS_UPDATE)
                })
                .catch((err) => {
        ...
        deleteInviteLink: function (inviteLink) {
            let apiFactory = new ApiApiFactory()
            apiFactory
                .destroyInviteLink(inviteLink.id)
                .then((r) => {
        ...
        resetInheritance: function () {
            axios
                .get(resolveDjangoUrl("api_reset_food_inheritance"))
                .then((r) => {
                    StandardToasts.makeStandardToast(this, StandardToasts.SUCCESS_UPDATE)
                })
                .catch((err) => {
