 ...
 * number-range filter from RECIPE_FILTER_DEFS into the correct snake_case
 * query param in the HTTP request.
 *
 * The bug this locks in: recipe_search.py accepted working_time_gte etc.,
 * but they were missing from the OpenAPI schema so the generated client never
 * emitted them — all 566 recipes returned regardless of the filter value.
 * Adding params to the schema is not enough; this test proves the client
 * actually sends them.
 */
