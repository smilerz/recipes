    ...
    <v-app>
        ...
        <v-navigation-drawer v-if="!mobile && useUserPreferenceStore().isAuthenticated && !useUserPreferenceStore().isPrintMode">
            <v-list>
                <menu-user-info></menu-user-info>
                <v-divider></v-divider>
                <component :is="item.component" v-bind="{prependIcon: item.prependIcon, title: item.title, to: item.to, href: item.href, onClick: item.onClick}" :key="item.title" v-for="item in useNavigation().getNavigationDrawer()"></component>
        ...
        <v-bottom-navigation grow v-if="useUserPreferenceStore().isAuthenticated && mobile && !useUserPreferenceStore().isPrintMode">
            ...
            <v-btn value="nearby">
                ...
                <v-bottom-sheet activator="parent" close-on-content-click>
                    <v-list nav>
                        <component :is="item.component" v-bind="{prependIcon: item.prependIcon, title: item.title, to: item.to, href: item.href, onClick: item.onClick}" :key="item.title" v-for="item in useNavigation().getBottomNavigation()"></component>
                    </v-list>
                </v-bottom-sheet>
            </v-btn>
        </v-bottom-navigation>
